	...
	<tbody>
		...
		foreach ($result as $value) {
			...
			foreach ($value['child'] as $item) {
				echo '<li class="item-child">'.highlight_phrase($item['name'], $q, '<span style="color:red; font-weight: bold;">', '</span>');
			...
			foreach ($value['child'] as $item_child) {
				if(isset($item_child['child1'])){
					foreach ($item_child['child1'] as $item_child_child) {
						echo '<li class="item-child">'.highlight_phrase($item_child_child['name'], $q, '<span style="color:red; font-weight: bold;">', '</span>');
