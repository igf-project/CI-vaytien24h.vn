...
<form id="frm_list" method="get" action="<?php echo $base_url ?>index/">
...
<table class="table table-striped table-bordered table-group" cellspacing="0" width="100%">
	...
	<tbody>
		...
		foreach ($users as $value) {
			...
			echo '<tr>
			<td>'.$i.'</td>
			<td class="tbl_actions">'.$value['name'];
			echo '<a href="'.$base_url.'edit/'.$value['id'].'" title="Sửa"><i class="fa fa-pencil-square-o blue" aria-hidden="true"></i>Edit</a>
			...
			foreach ($value['child'] as $item) {
				echo '<li class="item-child">'.$item['name'];
