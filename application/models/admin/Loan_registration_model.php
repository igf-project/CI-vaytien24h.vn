<?php
Class Loan_registration_model extends CI_Model {
	private $table;
		
	function __construct() {
		parent::__construct();
	
		$this->table = 'tbl_loan_registration';
	}
	...
	public function rows($where=NULL, $order=NULL, $page=0, $psize=25, $par_id=0) {
		...
		// Add Borrow_group to array
		$count = count($result);
        for ($i=0;$i < $count; $i++) {
        	$borrow_group_name = $this->get_name_borrow_group($result[$i]['type']);
            $result[$i]['borrow_group'] = $borrow_group_name;
        } 
		return $result;
	}

	// Get name post_group
	public function get_name_borrow_group($id){
		$this->db->select('name');
		$this->db->where('id', $id);
		$query = $this->db->get('tbl_borrow_group')->row_array();
		return $query['name'];
	}
